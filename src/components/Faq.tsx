export function Faq() {
  return (
    <>
      <h1 className="pt-24 text-slate-950 text-center font-bold text-4xl">FAQ</h1>
      <div className="py-4 mx-32">
        <p className="text-slate-950 text-2xl font-bold px-2">
          How to install the profiles?
        </p>
        <p className="text-slate-950 text-xl px-2">
          Copy the .ini and .py files from the zip archive into "C:\Users\%yourusername%\AppData\Roaming\Virtuali\GSX\MSFS". If the profile requires anything else to be done (for example installing an exclusion file), this will be outlined in the readme.txt file located in the zip file
        </p>
      </div>
      <div className="py-4 mx-32">
        <p className="text-slate-950 text-2xl font-bold px-2">
          How to install the liveries?
        </p>
        <p className="text-slate-950 text-xl px-2">
          Copy the package from the zip file into your community folder like with any other msfs package
        </p>
      </div>
      <div className="py-4 mx-32">
        <p className="text-slate-950 text-2xl font-bold px-2">
          Question
        </p>
        <p className="text-slate-950 text-xl px-2">
          Lorem ipsum dolor sit, amet consectetur adipisicing elit. Odit rerum dolorem id sed. Eos asperiores ea facere eveniet modi quae, molestiae placeat saepe laborum. Minus eos nisi qui ratione vel!
        </p>
      </div>
    </>
  )
}